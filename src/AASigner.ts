...
export class AASigner extends Signer {
    ...
    const waitPromise = new Promise<TransactionReceipt>((resolve, reject) => {
      let listener = async function (this: any, ...param: any): Promise<void> {
        ...
        if (!event.args.success) {
          ...
          console.log('revertReasonEvents', revertReasonEvents)
          if (revertReasonEvents.length > 0) {
            const revertReason = revertReasonEvents[0].args.revertReason
            console.log('Reverted with:', revertReason)
          }
          // eslint-disable-next-line @typescript-eslint/strict-boolean-expressions
  ...
  async sendTransaction(transaction: Deferrable<TransactionRequest>): Promise<any> {
    ...
    try {
      ...
      // Attempt to decode the revert reason if available
      if (error.data) {
